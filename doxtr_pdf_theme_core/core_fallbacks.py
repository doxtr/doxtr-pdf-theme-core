...
\definecolor{sphinxTableRowColorHeader}{rgb}{<< t_conf.header_background_color_cmyk >>}
...
\setlength{\aboverulesep}{0pt}\setlength{\belowrulesep}{0pt}\setlength{\extrarowheight}{0.75ex}\arrayrulecolor{black!30}\def\sphinxtoprule{\arrayrulecolor{ddconttitlefgtable}\toprule\arrayrulecolor{black!30}\sphinxtabletoprulehook}\def\sphinxmidrule{\arrayrulecolor{ddtableheaderbg}\midrule\arrayrulecolor{black!30}}\def\sphinxbottomrule{\arrayrulecolor{ddconttitlefgtable}\bottomrule\arrayrulecolor{black!30}}
...
# --- URL Line-Break Guard Absolute Fallback ---
# Injected only when a child theme is broken or missing its
# latex_styles/url_break/default.tex_t override (tier-6 of template
# resolution).  This mirrors the overridable .tex_t fragment shipped in
# latex_styles/url_break/default.tex_t — keep the two in sync.
#
# This is a Jinja2 template fragment (rendered through the same engine as
# every other style type), so it uses the core's custom control-flow
# delimiters: <% ... %>. The only variable it consumes is
# ``doxtr_url_break_aggressive`` (bool); it performs no << ... >> interpolation.
#
# Behaviour (functionally equivalent to the original inline preamble guard):
#   * \Urlmuskip adds a little inter-character stretch so the line-breaker
#     has slack to work with (avoids ragged overfulls even mid-token).
#   * \UrlBreaks is extended with a structural URL break-after character set.
#   * When doxtr_url_break_aggressive is True, alphabetic + numeric break
#     points are added so even unbroken alphanumeric tokens (base64 blobs,
#     percent-encoded parameters, long identifiers) can wrap.
DEFAULT_URL_BREAK_STYLE = r"""
%% ----------------------------------------------------
...
%%     latex_styles/url_break/default.tex_t in your theme/project.
